...
public class BasicCaching implements CachingSystem {
    ...
    private static final long REASONABLE_DISK_SIZE = 50 * 1024 * 1024; // 1 MB
    ...
    public static BasicCaching fromCtx(Context context) {
        return new BasicCaching(
                new File(context.getCacheDir(), "cashNice"),
    ...
    public void clear() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (diskCache.getDirectory().exists()) {
                    if (diskCache.getDirectory().isDirectory()) {
                        String[] children = diskCache.getDirectory().list();
                        for (int i = 0; i < children.length; i++) {
                            new File(diskCache.getDirectory(), children[i]).delete();
                        }
                    }
                }
            }
    ...
    public void remove(URL url) {
        memoryCache.remove(urlToKey(url));
        try {
            diskCache.remove(urlToKey(url));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
