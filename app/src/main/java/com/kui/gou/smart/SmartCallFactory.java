...
public class SmartCallFactory extends CallAdapter.Factory {
    private final BasicCaching cachingSystem;
    private final Executor asyncExecutor;

    public SmartCallFactory(BasicCaching cachingSystem) {
        this.cachingSystem = cachingSystem;
        this.asyncExecutor = new AndroidExecutor();
    }

    public SmartCallFactory(BasicCaching cachingSystem, Executor executor) {
        this.cachingSystem = cachingSystem;
        this.asyncExecutor = executor;
    }

    public BasicCaching getCachingSystem() {
...
//                Log.e("buildRequestFromCall"+exc.toString());
        ...
        public void enqueue(boolean cache, final Callback<T> callback) {
            if (buildRequest().method().equals("GET") && cache) {
                enqueueWithCache(callback);
            } else {
                baseCall.enqueue(new Callback<T>() {
                    @Override
